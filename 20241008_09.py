...
def log_data():
    ...
    try:
        while True:
            ...
            local_time = datetime.fromtimestamp(current_time, pytz.timezone('America/New_York'))  # Change timezone as needed
            timestamps.append(local_time)
            data_df.loc[len(data_df)] = [
                local_time,
                current_temp_1,
                (current_temp_1 - last_temp_1) / interval if last_temp_1 is not None else 0,
                current_temp_2,
                (current_temp_2 - last_temp_2) / interval if last_temp_2 is not None else 0,
...
def update_plots(df):
    ...
    min_temp = min(min(df['Temperature Sensor 1 (°F)'].min(), df['Temperature Sensor 2 (°F)'].min()), 0) - 5
    max_temp = max(max(df['Temperature Sensor 1 (°F)'].max(), df['Temperature Sensor 2 (°F)'].max()), 0) + 5
    min_rate = min(min(df['Rate of Change Sensor 1 (°F/s)'].min(), df['Rate of Change Sensor 2 (°F/s)'].min()), 0) - 0.5
    max_rate = max(max(df['Rate of Change Sensor 1 (°F/s)'].max(), df['Rate of Change Sensor 2 (°F/s)'].max()), 0) + 0.5

    # Ensure lengths match for plotting
    plot_timestamps = df['Time']

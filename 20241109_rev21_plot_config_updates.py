...
plt.ion()
# adjust size of plots, upstairs monitor was 14,12.
...
#            local_time = datetime.fromtimestamp(current_time, pytz.timezone('America/New_York')).strftime("%H:%M")
...
def update_plots(df):
    ...
    last_roctemp1 = plot_data['Rate of Change Sensor 1 (°F/h)'].iloc[-1]
    last_roctemp2 = plot_data['Rate of Change Sensor 2 (°F/h)'].iloc[-1]
    last_avgtemp1 = plot_data['Moving Average Rate of Change Sensor 1 (°F/h)'].iloc[-1]
    last_avgtemp2 = plot_data['Moving Average Rate of Change Sensor 2 (°F/h)'].iloc[-1]
    #last_avgtempavg = plot_data['Average Rate of Change (°F/h)'].iloc[-1]
    ...
    plot_configs = [
        ...
        },
        {
            ...
            # modify to bounds of viewing choice
            "ylim": (-500, 1000),
            "last_avgpoint": f"Last T1: {last_avgtemp1:.2f} °F/hr\nLast T2: {last_avgtemp2:.2f} °F/hr"
        },
        {
            "ax": axs[2],
            "data": [plot_data['Rate of Change Sensor 1 (°F/h)'], plot_data['Rate of Change Sensor 2 (°F/h)']],
            "labels": ['Raw RoC T1 (°F/h)', 'Raw RoC T2 (°F/h)'],
            "title": 'Raw Rate of Change of Temperature (°F/h)',
            "ylabel": 'Raw Rate of Change (°F/h)',
            #"ylim": (min_rate, max_rate),
            "ylim": (-500, 1000),
            "last_rocpoint": f"Last T1: {last_roctemp1:.2f} °F\nLast T2: {last_roctemp2:.2f} °F"
        },
    ]
    ...
    for config in plot_configs:
        ...
        config["ax"].legend(loc='upper left')  # Move legend to upper left
        config["ax"].grid()
        # turn this on to inrease x axis ticks, off to return to the 5 that are showing up
        config["ax"].xaxis.set_major_locator(MaxNLocator(integer=True, prune='lower', nbins=12))  # Set nbins to 12 for more ticks
        # Format x-axis to only show time (HH:MM) without the day
        config["ax"].xaxis.set_major_formatter(DateFormatter('%H:%M'))

        
        
# Annotate the last data point for the first plot
        if 'last_point' in config:
            config["ax"].text(0.35, 0.95, config["last_point"], transform=config["ax"].transAxes,
                              fontsize=10, verticalalignment='top', horizontalalignment='right',
                              bbox=dict(boxstyle='round', facecolor='white', alpha=0.5))
                              
        if 'last_rocpoint' in config:
            config["ax"].text(0.45, 0.95, config["last_rocpoint"], transform=config["ax"].transAxes,
                              fontsize=10, verticalalignment='top', horizontalalignment='right',
                              bbox=dict(boxstyle='round', facecolor='white', alpha=0.5))
                              
        if 'last_avgpoint' in config:
            config["ax"].text(0.45, 0.95, config["last_avgpoint"], transform=config["ax"].transAxes,
                              fontsize=10, verticalalignment='top', horizontalalignment='right',
                              bbox=dict(boxstyle='round', facecolor='white', alpha=0.5))
    plt.draw()

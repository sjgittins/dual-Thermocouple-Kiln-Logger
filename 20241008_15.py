...
update_interval = 30   # Time interval to update CSV in seconds
smoothing_window = 2   # Reduced rolling average window size for less filtering
...
columns = ['Time', 'Temperature Sensor 1 (°F)', 'Rate of Change Sensor 1 (°F/h)',
           ...
           'Average Temperature (°F)', 'Filtered Rate of Change Sensor 1 (°F/h)', 
           'Filtered Rate of Change Sensor 2 (°F/h)', 'Filtered Average Rate of Change (°F/h)',
           'Average Rate of Change (°F/h)']  # New column added
    ...
    try:
        while True:
            ...
            if len(data_df) % update_interval == 0:
                # Calculate filtered values using rolling mean
                data_df['Filtered Rate of Change Sensor 1 (°F/h)'] = data_df['Rate of Change Sensor 1 (°F/h)'].rolling(window=smoothing_window).mean()
                data_df['Filtered Rate of Change Sensor 2 (°F/h)'] = data_df['Rate of Change Sensor 2 (°F/h)'].rolling(window=smoothing_window).mean()
                data_df['Filtered Average Rate of Change (°F/h)'] = (data_df['Filtered Rate of Change Sensor 1 (°F/h)'] + data_df['Filtered Rate of Change Sensor 2 (°F/h)']) / 2
...
def update_plots(df):
    ...
    min_filt_rate = plot_data[['Filtered Rate of Change Sensor 1 (°F/h)', 'Filtered Rate of Change Sensor 2 (°F/h)']].min().min() - 5
    max_filt_rate = plot_data[['Filtered Rate of Change Sensor 1 (°F/h)', 'Filtered Rate of Change Sensor 2 (°F/h)']].max().max() + 5
    ...
    if np.isnan(min_filt_rate) or np.isnan(max_filt_rate) or np.isinf(min_filt_rate) or np.isinf(max_filt_rate):
        min_filt_rate, max_filt_rate = -10, 10  # Default limits
    ...
    plot_configs = [
        ...
        {
            "ax": axs[1],
            "data": [plot_data['Filtered Rate of Change Sensor 1 (°F/h)'], plot_data['Filtered Rate of Change Sensor 2 (°F/h)'], plot_data['Filtered Average Rate of Change (°F/h)']],
            "labels": ['Filtered Rate of Change Sensor 1 (°F/h)', 'Filtered Rate of Change Sensor 2 (°F/h)', 'Filtered Average Rate of Change (°F/h)'],
            "title": 'Rate of Change of Temperature (°F/h)',
            "ylabel": 'Rate of Change (°F/h)',
            "ylim": (min_filt_rate, max_filt_rate)
        },
        {
            "ax": axs[2],
